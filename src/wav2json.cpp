...
{
	return clamp<float>(out_min + (out_max - out_min) * (x - in_min) / (in_max - in_min), out_min, out_max);
}
...
/* function to filter out invalid channels
* lambdas would be nicer, but hey, it's 2015 :)
* */
struct filter_invalid_channels
{
	...
	{
		if ((channel == Options::MID || channel == Options::SIDE || channel == Options::RIGHT || channel == Options::MIN || channel == Options::MAX) && wav.channels() == 1)
		{
...
/*
  ...
  */
void compute_waveform(const SndfileHandle& wav, std::ostream& output_stream, size_t samples, Options::Channels channels, bool use_db_scale, float db_min, float db_max, progress_callback_t progress_callback)
{
	using std::size_t;
	using std::cerr;
	using std::endl;
	...
	channels.erase(std::remove_if(channels.begin(), channels.end(), filter), channels.end());
	...
	for (size_t x = 0; x < samples; ++x)
	{
		...
		for (size_t channel_idx = 0; channel_idx != channels.size(); ++channel_idx)
		{
			...
			float y = 0;
			if (use_db_scale == true)
			{
				y = map2range(float2db(max / (float)sample_scale<sample_type>::value), db_min, db_max, 0, 1);
			}
			else
			{
				y = map2range(max, 0, sample_scale<sample_type>::value, 0, 1);
			}

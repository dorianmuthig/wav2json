...
template <> struct sample_scale < short >
{
	...
	template <> struct sample_scale < float >
	{
		...
		template <typename T> T compute_sample(const std::vector<T>& block, int i, int n_channels, Options::Channel channel)
		{
			switch (channel)
			{
				case Options::LEFT:
		...
		float compute_waveform(const SndfileHandle& wav, std::ostream& output_stream, size_t samples, Options::Channel channel, bool use_db_scale, float db_min, float db_max, progress_callback_t progress_callback)
		{
			...
			{
				std::cerr << "Warning: you're trying to generate output for channel '" << channel << "', but the input has only one channel. Falling back to the left channel." << std::endl;
			...
			for (size_t x = 0; x < samples; ++x)
			{
				...
				if (x != samples - 1)
				{
					//only write out comma, if this is not the last sample
					output_stream << ",";
				}
